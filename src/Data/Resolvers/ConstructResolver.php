...
namespace Arkye\Support\Data\Resolvers;
...
class ConstructResolver
{
  ...
  public function execute(BaseData &$class, mixed ...$payloads)
  {
    if ($data = $this->createFromCustomCreationMethod($class::class, $payloads)) {
      return $data;
    }
    ...
    $properties->each(fn($value, $key) => $class->{$key} = $value);
  }
